...
class ErrorParserTest {
    ...
    inner class ErrorTypeTest {
        ...
        @Test
        @DisplayName("Maps status with colon separator to TIMEOUT")
        fun mapsStatusWithColonToTimeout() {
            val error = Exception("GATT error status:8")

            assertThat(error.errorType()).isEqualTo(ConnectionErrorType.TIMEOUT)
        }

        @Test
        @DisplayName("Maps status from nested cause message to TIMEOUT")
        fun mapsNestedCauseMessageToTimeout() {
            val error = Exception("outer", Exception("inner status=8 timeout"))
    ...
    inner class MapDiscoverServicesErrorTest {
        ...
        @Test
        @DisplayName("Returns already-connected code for wrapped BleAlreadyConnectedException")
        fun returnsAlreadyConnectedCodeForWrappedCause() {
            val error = Exception("wrapper", BleAlreadyConnectedException("AA:BB:CC:DD:EE:FF"))
